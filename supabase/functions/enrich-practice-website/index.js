...
import { enrichWebsiteDentist } from '../_shared/enrichment.js'
...
Deno.serve(async (req) => {
  ...
  const dentistIds = body.dentist_id ? [body.dentist_id] : await selectDentistsForWebsite(supabase, limit)
  ...
  for (const dentistId of dentistIds) {
    try {
      ...
      const result = await enrichWebsiteDentist(supabase, dentistId)
      summary.processed += 1
...
async function selectDentistsForWebsite(supabase, limit) {
  const { data, error } = await supabase
    .from('dentists')
    .select('id')
    .not('website', 'is', null)
    .or('public_email.is.null,practice_domain.is.null,owner_confidence.lt.50,education_school.is.null')
